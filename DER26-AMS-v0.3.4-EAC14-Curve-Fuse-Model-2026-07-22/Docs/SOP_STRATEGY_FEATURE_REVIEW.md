...
## D. Preliminary EAC14-80 curve observer

The installed main fuse is an Eaton EAC14-80-SCT. Eaton publishes an 80 A
rating, approximately 1.03 mOhm typical cold resistance, 150 mV typical voltage
drop, and 8020 A2s typical melting I2t measured at 10 times rated current. That
8020 A2s value is a single typical 800 A point, not a universal multi-second
budget.

v0.3.4 therefore uses a rounded digitization of the EAC14-80 **current-versus-
time centerline** over the charted 0.01-to-100-second range. The 800 A point is
anchored to `8020 / 800^2 = 0.01253125 s`. The model also uses the published
ambient-temperature derating curve, but intentionally credits no cold-
temperature uplift.

The observer state is dimensionless utilization rather than stored A2s:

```text
t_usable(I,T) = curve_time_fraction * t_typical(I_equivalent_25C)
x_dot = -x / tau + source_rate(I_equivalent_25C)
source_rate chosen so constant current reaches x=1 at t_usable from cold
```

The production discrete update is intentionally conservative relative to the
independent exact zero-order-hold oracle. For every horizon, the applied cap is:

```text
min(static 118/80/70/70 A cap, curve-derived dynamic cap)
```

The default `curve_time_fraction` is 0.25. That is a commissioning margin only;
it is not a substitute for guaranteed-minimum manufacturer data. Currents below
the lowest digitized point and above the upper point are explicitly flagged as
extrapolated. Because the hardware has no diagnosed fuse-temperature sensor,
the hottest cell-surface temperature plus 15 degC remains an explicitly flagged
proxy.

The curve can never add current. At utilization >= 1 it requests zero until the
state cools below 0.5. After MCU startup it remains shadow-only until both the
model-validation build gate and the initial-state requirement are satisfied. A
reboot during a hot run cannot manufacture authoritative fuse headroom.

The published curve is a typical centerline. Installed-fuse, holder, busbar,
cable, contactor, ambient, pulse-cycling, aging, and power-interruption tests are
still required. Guaranteed-minimum Eaton data is requested in
`EAC14_80_MANUFACTURER_DATA_REQUEST.md`. Keep
`AMS_FUSE_MODEL_VALIDATED=0` until that evidence is closed.
