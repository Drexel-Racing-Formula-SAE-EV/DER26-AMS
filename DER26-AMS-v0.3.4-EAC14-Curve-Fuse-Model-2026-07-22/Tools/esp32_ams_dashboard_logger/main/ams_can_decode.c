...
static bool is_power_id(uint32_t id)
{
    return ((id >= AMS_POWER_CAN_ID_DCL) &&
           ...
           (id == AMS_POWER_CAN_ID_BINDINGS);
}

static uint8_t power_slot(uint32_t id)
{
    if(id == AMS_POWER_CAN_ID_STRATEGY)
    {
        return 4u;
    }
    if(id == AMS_POWER_CAN_ID_BINDINGS)
    {
        return 5u;
    }
    return (uint8_t)(id - AMS_POWER_CAN_ID_DCL);
}
                           ...
                           uint32_t now_ms)
{
    ...
    switch(frame->id)
    {
        ...
        case AMS_POWER_CAN_ID_BINDINGS:
        {
            const uint8_t binding[6] = {
                (uint8_t)(d[1] >> 4u), (uint8_t)(d[1] & 0x0Fu),
                (uint8_t)(d[2] >> 4u), (uint8_t)(d[2] & 0x0Fu),
                (uint8_t)(d[3] >> 4u), (uint8_t)(d[3] & 0x0Fu)
            };
            const uint8_t segment[6] = {
                (uint8_t)(d[4] >> 4u), (uint8_t)(d[4] & 0x0Fu),
                (uint8_t)(d[5] >> 4u), (uint8_t)(d[5] & 0x0Fu),
                (uint8_t)(d[6] >> 4u), (uint8_t)(d[6] & 0x0Fu)
            };
            for(uint8_t index = 0u; index < 6u; index++)
            {
                if((binding[index] > 14u) ||
                   !((segment[index] <= 4u) || (segment[index] == 0x0Fu)))
                {
                    state->malformed_frames++;
                    return false;
                }
            }
            state->envelope_discharge_binding[0] = binding[0];
            state->envelope_discharge_binding[1] = binding[1];
            state->envelope_discharge_binding[2] = binding[2];
            state->envelope_charge_binding[0] = binding[3];
            state->envelope_charge_binding[1] = binding[4];
            state->envelope_charge_binding[2] = binding[5];
            state->envelope_discharge_segment[0] = segment[0];
            state->envelope_discharge_segment[1] = segment[1];
            state->envelope_discharge_segment[2] = segment[2];
            state->envelope_charge_segment[0] = segment[3];
            state->envelope_charge_segment[1] = segment[4];
            state->envelope_charge_segment[2] = segment[5];
            break;
        }
