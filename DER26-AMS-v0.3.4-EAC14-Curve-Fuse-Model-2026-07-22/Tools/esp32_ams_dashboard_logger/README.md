...
| `0x684` | Active mission-selected DCL/current, discharge power, flags, binding and limiting segment |
| `0x685` | Active CCL/current, charge power, flags, binding and limiting segment |
...
| `0x689` | Advisory mission profile, AMS-owned fuse utilization, thermal resource and R0 bootstrap progress |
| `0x68A` | Advisory per-horizon/per-direction binding causes and limiting segments |
...
required fail-zero power contract; loss of advisory `0x689` or `0x68A` does
not invalidate the hard bundle. `/api/state` reports whether `0x68A` metadata
is fresh and exposes three-element discharge/charge binding and limiting-
segment arrays ordered as `0.1/10/30 s`.
