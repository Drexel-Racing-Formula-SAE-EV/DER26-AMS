...
def run_one(binary: Path, trace: Path, startup: str, reset: str,
            usable: float, tau: float) -> dict[str, str]:
...
def main() -> None:
        ...
        for startup in startups:
            for usable in (0.25, 0.50, 0.75):
                for tau in (120.0, 300.0, 600.0):
                    cases.append((trace, startup, "unknown", usable, tau))
    ...
    for reset_name in ("reset_pre_exhaust.csv", "reset_midrun.csv"):
        ...
        for reset in ("unknown", "known-cold", "seeded:0.5", "seeded:0.8", "restore"):
            for usable in (0.25, 0.50, 0.75):
                cases.append((reset_trace, "cold-soak", reset, usable, 300.0))

    rows: list[dict[str, str]] = []
    for index, (trace, startup, reset, usable, tau) in enumerate(cases, 1):
