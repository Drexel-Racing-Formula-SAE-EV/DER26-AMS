...
static void decode_rx_header(const uint8_t buf[13], mcp2515_frame_t *frame)
{
    bool extended = (buf[1] & SIDL_EXIDE) != 0u;

    frame->extended = extended;
    if(extended)
    {
        frame->id = (((uint32_t)buf[0]) << 21u) |
                    (((uint32_t)(buf[1] & 0xE0u)) << 13u) |
    ...
    {
        frame->id = (uint16_t)(((uint16_t)buf[0] << 3u) |
                               ((uint16_t)buf[1] >> 5u));
    }

    frame->dlc = buf[4] & 0x0Fu;
    if(frame->dlc > 8u)
    {
        frame->dlc = 8u;
    }
...
esp_err_t mcp2515_init(void)
{
    ...
    /* 250 kbit/s with an 8 MHz MCP2515 crystal:
    ...
    /* Accept all standard/extended frames into RXB0/RXB1. */
