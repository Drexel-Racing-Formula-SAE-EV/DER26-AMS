...
void cli_task_fn(void *arg)
{
    ...
    cli_printline(local_cli,
                  "EVAL LOCK: 1x ADBMS6830B / 16 cells / String B; BMS_OK, balance, mux, open-wire, APM and fans disabled");
#endif
...
int get_status(int argc, char *argv[])
{
    ...
    ret |= cli_printline(cli,
                         "Eval order: spi pins -> spi probeb -> spi sid -> spi stat -> volt -> bringup adbms6830; BMS_OK release unavailable");
#else
...
static int get_spi_debug_locked(int argc, char *argv[])
{
    ...
    if((argc >= 2) && (argv[1] != NULL))
    {
        ...
        {
            adbms6830_scope_mode_t requested_mode =
                cli_adbms_scope_default_mode;
            if((argc >= 4) && (argv[3] != NULL) &&
               !cli_parse_scope_mode(argv[3], &requested_mode))
            {
    ...
    uint8_t ic_count = smb_ic_count(smb);
	...
	#if AMS_EVAL_ADBMS6830_BMSW
	            probe_status =
	                adbms6830_spi_probe_rdcfga_on_string(smb, STRING_B);
	            adbms6830_resync_command_counter_tracking(smb);
	            snprintf(outline, CLI_LINESZ,
	                     "ADBMS6830 RDCFGA CS_B/stringB probe status: %s",
        ...
        else if(!strcmp(argv[1], "diagclear"))
        {
	        ...
	        else if(!strcmp(argv[1], "cfgchk"))
	        {
	            ...
	            probe_status = adbms6830_spi_probe_rdcfga(smb);
	            snprintf(outline, CLI_LINESZ,
	                     "RDCFGA integrity status: %s (monitor-only reset defaults; no expected-byte compare)",
	                     cli_hal_status_str(probe_status));
	#endif
	            ret |= cli_printline(cli, outline);
	        }
...
int get_bringup(int argc, char *argv[])
{
...
#if AMS_EVAL_ADBMS6830_BMSW
        ret |= cli_printline(cli, "eval start: spi pins -> spi probeb -> spi sid -> spi stat -> volt");
#else
...
#if AMS_EVAL_ADBMS6830_BMSW
        ret |= cli_printline(cli, "pin_check: run spi pins; active eval path is CS_B/PE4");
...
#if AMS_EVAL_ADBMS6830_BMSW
        ret |= cli_printline(cli, "next: spi pins -> spi cs b pulse 10 -> spi scope b read 20");
		ret |= cli_printline(cli, "then: spi probeb -> spi sid -> spi stat -> volt");
#else
    ...
    if(!strcmp(mode, "ready"))
    {
        ...
        bool release_ok = (AMS_EVAL_ADBMS6830_BMSW == 0) &&
                          voltage_ready && temp_ready && current_ready &&
                          heartbeat_ready && hard_ready;
        ...
        ret |= cli_printline(cli, outline);
#if AMS_EVAL_ADBMS6830_BMSW
        ret |= cli_printline(cli, "note: monitor health only; BMS_OK is permanently locked low");
#else
    ...
    if(!strcmp(mode, "evidence"))
    {
        ...
        ret |= cli_printline(cli, "2 spi pins; scope MCU SCK/MOSI/MISO and CS_B/PE4");
        ret |= cli_printline(cli, "3 spi clear; spi enable; spi cs b pulse 10; spi scope b read 20");
		ret |= cli_printline(cli, "4 spi probeb; spi sid; spi stat; spi cfgchk");
        ret |= cli_printline(cli, "5 volt; bringup adbms6830; bringup ready");
#else
        ret |= cli_printline(cli, "1 status; bringup board; bmsok status; fault");
...
int bmsok_control(int argc, char *argv[])
{
    ...
    if((argc >= 2) && (argv[1] != NULL))
    {
        if(!strcmp(argv[1], "release") || !strcmp(argv[1], "enable"))
        {
            ...
            set_bms(false);
            ret |= cli_printline(cli,
                                 "BMS_OK release blocked: eval-board profile is permanently output-inhibited");
...
int balance_control(int argc, char *argv[])
{
    ...
    if((argc >= 2) && (argv[1] != NULL))
    {
...
#else
            ret |= cli_printline(cli,
                                 "Balancing remains compile-time disabled; no ADBMS write sent");
#endif
        }
        else if(!strcmp(argv[1], "release") || !strcmp(argv[1], "enable"))
        {
            ...
            data->balance_inhibit = true;
            ret |= cli_printline(cli,
                                 "Balancing release blocked: eval-board profile has no balance-write path");
#elif AMS_ENABLE_SERVICE_CLI
        ...
        else if(!strcmp(argv[1], "clear"))
        {
            ...
            data->balance_inhibit = true;
            ret |= cli_printline(cli,
                                 "Balance clear not sent: eval-board profile is monitor-only");
#endif
        }
        else if(strcmp(argv[1], "status"))
        {
...
int set_state(int argc, char *argv[])
{
    ...
    else if(argc == 2)
    {
        ...
        return cli_printline(cli,
                             "AMS state change blocked: eval-board profile remains in START/monitor mode");
#elif !AMS_ENABLE_SERVICE_CLI
