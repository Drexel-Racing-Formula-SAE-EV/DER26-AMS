...
bool accumulator_final_ring_topology_valid(const accumulator_t *dev)
{
    ...
    const adbms_string expected_smb_string = STRING_B;
#else
    const adbms_string expected_smb_string = STRING_A;
#endif

    if((dev == NULL) ||
       (dev->smb.num_ics != NSMBS) ||
       (dev->smb.physical_chain_count != AMS_ADBMS_PHYSICAL_CHAIN_COUNT) ||
       (dev->smb.ics_capacity != NSMBS) ||
       (dev->smb.ics != dev->smb_ics) ||
       (dev->smb.string != expected_smb_string) ||
       (dev->smb.write_string != expected_smb_string) ||
       (dev->smb.hspi == NULL) ||
       (dev->smb.htim == NULL) ||
...
{
    /* PHAPOL/PHAPOL2 are strapped high on the DER26 AMS, selecting ADBMS6822
     * SPI mode 3. Reject a regenerated or mismatched Cube configuration
     * before sending wake/reset traffic to the eval fixture. */
    return (hspi != NULL) &&
...
uint16_t accumulator_balance_shadow_mask(const accumulator_t *dev, uint8_t seg)
{
    ...
    const adbms6830_asic *smb_ics = (dev->smb.ics != NULL) ?
                                      dev->smb.ics : dev->smb_ics;
    uint16_t mask = (uint16_t)(smb_ics[seg].tx_cfgb.dcc &
                               ACCUMULATOR_CELL_MASK);
					  ...
					  TIM_HandleTypeDef* htim)
{
	...
	/* Leave all driver objects in a known unusable state if a profile or
	...
	/* Full-ring builds use the leading String-A SMB subset. The eval fixture
	 * instead contains one String-B monitor and no APM. */
	dev->smb_init_status = adBms6830_init(&dev->smb,
	                                      NSMBS,
	                                      AMS_ADBMS_PHYSICAL_CHAIN_COUNT,
	                                      dev->smb_ics,
	                                      NSMBS,
	                                      hspi,
	                                      cs_port_a,
	...
	if(dev->smb_init_status == HAL_OK)
	{
		...
		adbms_spi_unlock();
	#else
		/* Monitor-only initialization already completed a PEC/counter/product-ID
		 * checked SID read. It intentionally has no expected CFGA/CFGB image. */
		const adbms6830_diag_health_t *health =
			adbms6830_diag_health_get(&dev->smb);
		const uint16_t expected_sid_mask = (uint16_t)((1u << NSMBS) - 1u);
		if((health == NULL) ||
		   (health->sid_valid_ic_mask != expected_sid_mask) ||
		   (health->sid_identity_mismatch_ic_mask != 0u))
		{
			dev->smb_init_status = HAL_ERROR;
		}
	#endif
...
//	adbms6830_wrcfgb(dev);
	...
	/* REFON is asserted in the verified full configuration but may remain at
	 * reset default in the eval monitor-only profile. Together with the checked
	 * wake inside the ADCV wrapper, this interval leaves more than the 4.4 ms
	 * worst-case reference wake time before ADCV in either profile. */
...
int accumulator_read_temp(accumulator_t *dev)
{
	 ...
	 * per-scan publication masks and never send COMM/mux traffic to unrelated
	 * eval-board GPIOs. */
...
#else
	if(!dev->smb_ready || !accumulator_final_ring_topology_valid(dev))
	{
		return ACCUMULATOR_STATUS_ERROR;
	}
...
void accumulator_update_voltage_stats_at(accumulator_t *dev, uint32_t now_ms)
{
    ...
    for(uint8_t ic = 0u; ic < ic_count; ic++)
    {
		...
		dev->pec_fail_voltage_mask[ic] =
			(uint16_t)(read_pec_mask & ACCUMULATOR_CELL_MASK);
