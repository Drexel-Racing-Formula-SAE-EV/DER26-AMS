...
The three `eval-adbms6830-*` targets are the focused gate for the physical
EVAL-ADBMS6830BMSW branch. They verify one monitor/16 cells on String B,
monitor-only initialization, the SPI6 configuration guard, and compile-time
lockout of BMS_OK, balancing, mux, open-wire, APM, fan and estimator paths. The
separate unit target also exercises the real ADBMS6830 driver translation in
monitor-only mode and confirms startup sends SRST plus checked RDSID, with no
CFGA/CFGB writes.
