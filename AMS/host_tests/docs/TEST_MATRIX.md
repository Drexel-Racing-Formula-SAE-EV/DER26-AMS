...
| `test_bringup_cli_apm_and_charger_phase_split` | Verifies `bringup apm2950`, `bringup charger-lv`, and `bringup charger-battery` keep APM debug-only, document `BYTE5/data[4]` charger polarity, expose the allow frame, and split LV charger checks from battery-connected checks. |
...
| Renode fake ADBMS6830 backend | `AMS_RENODE=1` enables a firmware-side fake ADBMS6830 chain for pre-hardware software-path checks. The `adbmsfake` CLI can reset the model, set nominal cells, inject OV/UV cells, corrupt PEC by IC mask, model missing IC responses, and inject command-counter mismatches. This validates AMS parser/fault logic, not real ADBMS6822/isoSPI electrical behavior. |
| Renode fake SMB temperature backend | `AMS_RENODE_FAKE_TEMP=1` enables `tempfake`, which writes modeled thermistor raw codes into the normal accumulator temperature path. It covers healthy SMB sensors, hot/cold thresholds, invalid raw channels, one-shot missing updates, and held-missing/stale behavior. |
| Renode fake DHAB current backend | `AMS_RENODE_FAKE_CURRENT=1` enables `currentfake`, which feeds modeled DHAB ADC counts into the normal current sensor conversion path. It covers zero-current midscale, signed discharge/charge current, raw ADC rail faults, ADC read failures, and 50A-vs-800A disagreement. |
| Renode fake charger CAN backend | `AMS_RENODE_FAKE_CHARGER=1` enables `chargerfake`, which exercises the real charger command payload, `BYTE5/data[4]` polarity, readback frames, RX timeout, hardware flag bits, and CAN TX failure handling before charger hardware is available. |
| Renode CAN capture and scenarios | `AMS_RENODE_CAN_CAPTURE=1` enables `canlog` for firmware-side TX counters and last-frame visibility. The `scenario` CLI sets healthy, OV, UV, hot, cold, charger timeout, charger TX-fail, charge-ready, and current-trip states for repeatable manual Renode smoke tests. |
