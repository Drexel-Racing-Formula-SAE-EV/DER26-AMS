                         ...
                         uint8_t payload[8])
{
    ...
    const uint8_t binding = (snapshot != NULL) ?
        (discharge ? snapshot->discharge_binding_1s :
                     snapshot->charge_binding_1s) :
        (uint8_t)AMS_SOP_BIND_INVALID_INPUT;
    uint8_t segment = (snapshot != NULL) ?
        (discharge ? snapshot->discharge_limiting_segment_1s :
                     snapshot->charge_limiting_segment_1s) :
                                   ...
                                   ams_power_can_envelope_t *envelope)
{
    ...
    envelope->discharge_current_a[0] = (float)payload[1];
    envelope->discharge_current_a[2] = (float)payload[2];
    envelope->discharge_current_a[3] = (float)payload[3];
    envelope->charge_current_a[0] = -(float)payload[4];
    envelope->charge_current_a[2] = -(float)payload[5];
    envelope->charge_current_a[3] = -(float)payload[6];
    return true;
}
