...
**Reviewed software baseline:** DER26 AMS firmware `0.5.26` / package `v2.6.23`, incorporating the v2.6.17 CAN scheduler corrections, v2.6.19 current-window/measurement-integrity fixes, v2.6.20 follow-up fixes, v2.6.21 migration-readiness closeout, v2.6.22 post-closeout regression fixes, and the v2.6.23 watchdog/CAN safety-policy closeout  
...
**Explicit non-action:** this document update does not start the Zephyr port. The v2.6.23 FreeRTOS baseline intentionally closes the previously unresolved watchdog and CAN bus-off safety policies after the v2.6.22 regression fixes. It does not retune battery-model parameters or estimator equations, change the CAN frame scheduler/IDs, or change the approved SPI String-B PE4 mapping. No full MiL rerun, ARM target build, flash, physical CAN bus-off test, watchdog-reset timing test, or new hardware validation is claimed for v2.6.23.
...
1. Freeze and fully identify firmware 0.5.26 / package v2.6.23 as the current FreeRTOS comparison baseline.
2. Close the remaining hardware-validation, evidence-reconciliation, target-timing, and target-build-provenance gates; do not reopen issues already fixed and regression-covered in 0.5.26.
...
- This plan update incorporates the v2.6.21 migration-readiness closeout, the v2.6.22 post-closeout fixes described in Section 5.2a, and the v2.6.23 policy closeout described in Section 5.2b. v2.6.23 changes watchdog/CAN safety-state behavior but does not retune battery models or estimator equations.
- No full MiL campaign was rerun. v2.6.23 changes supervisory/watchdog/CAN fault policy, not the estimator/SoP/SoH numerical models or calibration parameters; focused host safety/CAN/HIL regressions cover the changed behavior.
- No ARM target binary was built or flashed for v2.6.23 because `arm-none-eabi-gcc` is not available in the review environment.
- No physical CAN bus-off injection, IWDG reset-timing run, or new hardware reproduction is claimed for v2.6.23.
- Host validation passed the comprehensive injection suite, IWDG-enabled safety suite, CAN scheduler/transport regressions, CAN-backed HIL replacement test, profile gates, measurement-integrity tests, release-identity tests, and whole-source GCC analysis in bench and vehicle profiles.
...
|---|---|---|
| Former F-01 runtime/source version mismatch | **Closed before v2.6.19.** `app.h` aliases `VER_*` to the canonical `ams_version.h`; the behavior was introduced by firmware 0.5.23 and is preserved in the 0.5.26 comparison baseline. | Preserve a single canonical version/build-manifest path and verify shell/CAN/retained/artifact identity in Zephyr. |
...
The vehicle profile enables IWDG, although physical reset timing and persistence evidence remain target-validation gates. Firmware 0.5.26 closes F-05 with an explicit **software-liveness/integrity** policy.
...
- preserve the 0.5.26 uncertainty contract: a calibrated sample has known, nonzero uncertainty; zero and `UINT16_MAX` remain non-calibrated/unknown for estimator, SoP, SoH, and CAN quality decisions;
...
Firmware 0.5.26 also freezes the bus-off authority policy that Zephyr must reproduce:
...
The CLI is valuable during bring-up but too large to port wholesale in the first hardware milestone. It contains substantial product-specific diagnostics and uses bounded blocking UART writes from a low-priority thread. The 0.5.26 vehicle profile sets `AMS_ENABLE_CLI=0`, so the vehicle startup/ISR path does not initialize UART3, arm RX, or create the CLI task. Preserve that no-shell vehicle default; target link-map evidence should still confirm unreachable CLI sections are eliminated from the qualified image.
...
1. Freeze firmware 0.5.26 / package v2.6.23 as the FreeRTOS comparison baseline and archive the focused measurement-integrity, post-closeout, and migration-readiness regression results. Do not reopen F-01/F-02/F-03/F-04/F-10/F-13 or the v2.6.22 AUX2/freshness/profile-test defects as active blockers unless a later change regresses their gates.
...
4. Produce clean FreeRTOS DEBUG and deterministic RELEASE baseline ELF/MAP/HEX/BIN artifacts on the approved ARM toolchain using 0.5.26 / v2.6.23 with the AMS profile explicitly recorded. Archive `BUILD_PROVENANCE.txt`, `SHA256SUMS.txt`, `.su` files, and the exact toolchain version.
...
11. Freeze a list of byte-exact replay vectors and expected safety state transitions, including the v2.6.19/v2.6.20 measurement-integrity edge cases, v2.6.22 constituent-age/AUX2 recovery cases, and v2.6.23 watchdog/CAN state-dependent fault/recovery cases.
12. Promote the focused current-window/CAN-aging/uncertainty regressions, v2.6.21 CLI gates, v2.6.22 profile/provenance/AUX2/effective-age gates, and v2.6.23 watchdog/CAN policy gates into the dual-build parity suite where applicable.
...
- [x] Canonical runtime source versioning is unified in the 0.5.26 baseline; the eventual archived target artifact must still be checked against its manifest.
...
1. Freeze firmware 0.5.26 / package v2.6.23 as the comparison baseline. Archive the v2.6.21/v2.6.22 regression records plus the v2.6.23 watchdog/CAN policy closeout and host validation with that baseline.
2. Promote the v2.6.19/v2.6.20 current-window, calibration-provenance, CAN per-reading aging, and uncertainty-sentinel cases; the v2.6.22 effective-age/AUX2 cases; and the v2.6.23 watchdog/CAN state/timing/latch cases into the migration replay/parity corpus.
...
|---|---|
| Canonical firmware version is 0.5.26 in the updated baseline | `Core/Inc/ams_version.h:5-14` |
...
| State-dependent CAN bus-off policy, 500 ms discharge deadline, sticky policy latch, and START CAN-authority gate | `Core/Src/ext_drivers/canbus.c`; `Core/Src/tasks/error_task.c`; `Core/Src/tasks/canbus_task.c`; `Core/Inc/app.h` |
...
| `measurement/ams_measurement.c` | Preserve | Reuse double-buffer/pinning protocol and 0.5.26 current-window ordering/metadata semantics with injected short lock operations. |
