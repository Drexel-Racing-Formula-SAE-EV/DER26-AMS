...
**Review date:** 2026-09-04; migration-plan baseline updated 2026-09-06 after final pre-Zephyr freeze hardening
**Reviewed software baseline:** DER26 AMS firmware `0.5.30` / package `v2.6.27` software-freeze candidate, incorporating the v2.6.17 CAN scheduler corrections, v2.6.19 current-window/measurement-integrity fixes, v2.6.20 follow-up fixes, v2.6.21 migration-readiness closeout, v2.6.22 post-closeout regression fixes, the v2.6.23 watchdog/CAN safety-policy closeout, v2.6.24 event-time/telemetry/service-recovery fixes, v2.6.25 transport-settlement/fault-injection/soft-error/service-recovery hardening, v2.6.26 repeated-bus-off event sequencing, and v2.6.27 ISR/task ownership plus atomic service-recovery hardening
...
**Explicit non-action:** this document update does not start the Zephyr port. The v2.6.27 candidate preserves the established watchdog/CAN policy and adds final concurrency hardening: physical BOFF identity remains SCE-sequenced; charger ENABLE is revalidated at the bxCAN hardware-load boundary; shared ISR/task scalar status has explicit visibility; recovery settlement is tied to an unchanged BOFF sequence; and manual recovery commits transport plus application CAN-safety state atomically. It does not retune battery-model parameters or estimator equations, change CAN IDs/encoding, or change the approved SPI String-B PE4 mapping. Per user request the final v2.6.27 regression suites were not rerun during packaging, and no ARM target build, flash, physical CAN bus-off test, watchdog-reset timing test, or new hardware validation is claimed.
...
1. Treat firmware 0.5.30 / package v2.6.27 as the final FreeRTOS software-freeze candidate; freeze it as the comparison baseline after the recipient reruns the listed host gates.
2. After the v2.6.27 host rerun passes, close the remaining hardware-validation, evidence-reconciliation, target-timing, and target-build-provenance gates; do not reopen issues already fixed in the 0.5.30 source candidate without a demonstrated regression.
...
| Migration authorization | Source candidate complete; host rerun pending | Known code-fixable blockers and safety-policy ambiguities are closed in the 0.5.30 source candidate. Per user request the final suites are delegated to the recipient; once they pass, freeze this FreeRTOS baseline and begin the no-authority Zephyr skeleton. Hardware/timing/evidence gates still block parity and vehicle authority. |
...
- This plan update incorporates the v2.6.21 migration-readiness closeout, the v2.6.22 post-closeout fixes described in Section 5.2a, the v2.6.23 policy closeout described in Section 5.2b, the v2.6.24 event-time/service-recovery/tuning fixes described in Section 5.2c, and the v2.6.25 freeze hardening described in Section 5.2d.
- No full MiL campaign was rerun. v2.6.25 changes CAN supervisory/transport-settlement bookkeeping, fault-injection parity, soft-error aging, and manual-recovery transaction ordering; it does not change estimator/SoP/SoH numerical models or calibration parameters.
- No ARM target binary was built or flashed for v2.6.25 because `arm-none-eabi-gcc` is not available in the review environment.
- No physical CAN bus-off injection, IWDG reset-timing run, or new hardware reproduction is claimed for v2.6.25.
- Focused validation passed comprehensive `test`, fault-injection-enabled `safety-test`, CAN scheduler/transport regressions, measurement-integrity tests, profile/review/CAN-contract gates, and whole-source GCC analysis in bench and vehicle profiles with `-Wundef -Werror=undef`. A full `firmware-ci` invocation progressed through the main unit/system/CAN/APM gates but hit the review execution-time limit while building a later logger-contract target; no failure was observed before timeout.
...
### 5.2d Final freeze hardening fixed in v2.6.25

| Closed item | Status in firmware 0.5.28 / package v2.6.25 | Migration requirement |
|---|---|---|
| Delayed pre-bus-off completion accepted before CAN-task settlement | **Fixed.** The 500 ms supervisor accepts a protected completion only after transport recovery is fully settled: bus-off/recovery flags are clear, the controller is active, refresh/suspension/repeated-bus-off states are clear, and the post-recovery authority baseline has already advanced beyond old-controller completions. If the CAN task has not completed this settlement by 500 ms, the system fails low. | Zephyr parity must distinguish event timestamp from recovery-epoch settlement. A completion timestamp alone is insufficient; the old driver/controller epoch must be retired before delivery evidence can restore authority. |
| Bench bus-off fault injection drift | **Fixed.** Physical BOFF and `fault inject canbusoff` now share one event-recording path for timestamp/state capture, immediate authority revocation, CHARGE/BALANCE/HIL fail-low, recovery epoch, TX suspension, and mailbox abort. | Use one policy/event adapter for physical callbacks and test injection so validation never exercises a weaker state machine than production. |
| Repeated identical soft CAN errors aged from first code transition | **Fixed.** New ISR-observed occurrences refresh `can_last_error_tick` even if the HAL error bits match the previous error. The soft hold expires from the most recent observed fault. | Preserve occurrence freshness independently from error-code identity. Do not use code transitions as the only event detector. |
| Failed manual recovery partially cleared repeated-bus-off state | **Fixed.** Service recovery clears `tx_latched_inhibit` and its repeat window only after hardware/mailbox settlement succeeds; a failed recovery leaves the previous transport policy state intact. | Administrative recovery must be transactional from the safety-policy perspective: do not clear sticky transport evidence before driver settlement succeeds. |

Focused regression covers delayed CAN-task settlement plus a late old-epoch completion, post-settlement 499 ms acceptance, exact 500/501 ms rejection, wrap-around, physical-equivalent CLI injection, repeated ACK-error aging, and failed manual recovery. Comprehensive `test` and fault-injection-enabled `safety-test`, CAN scheduler/transport tests, measurement-integrity tests, profile/review/CAN-contract gates, and whole-source GCC analysis pass. A full `firmware-ci` invocation reached later logger-contract work before the review execution-time limit and showed no failure before timeout. No ARM target build, flash, physical bus-off injection, or target timing evidence is claimed.

### 5.2e Repeated-bus-off sequencing closed in v2.6.26

| Closed item | Status in firmware 0.5.29 / package v2.6.26 | Migration requirement |
|---|---|---|
| Clustered/recovery-pending BOFF events could collapse before the 10 Hz task | **Fixed.** CAN1 SCE owns physical BOFF identity, every genuine event receives an ISR-side monotonic sequence, the exact three-event/10-second sliding window is maintained at the event boundary, and the third event hard-latches TX/BMS low immediately. Task recovery consumes sequence deltas and later BOFFs do not move the original 500 ms DISCHARGE deadline. | Zephyr must preserve physical-event identity independently of sticky controller error words and task polling cadence. Repeated-event policy belongs at the event boundary, not in a lossy pending boolean. |
| Sticky HAL BOF could double-create event identity | **Fixed.** Target-side physical BOFF identity comes from the SCE/ERRI hardware condition before HAL sticky error accumulation; HAL ErrorCallback may suspend transport but does not invent another physical event. | Separate hardware event identity from accumulated driver error status in the Zephyr CAN adapter. |

### 5.2f Final ISR/task and service-recovery hardening in v2.6.27

| Closed/source-hardened item | Status in firmware 0.5.30 / package v2.6.27 candidate | Migration requirement |
|---|---|---|
| Charger ENABLE stale between scheduler commit and bxCAN load | **Fixed in source.** Task-context charger ENABLE is revalidated against CHARGE state, BMS state, shutdown pending, and TX failure inside the same short critical section as the actual mailbox load. A stale enable is rewritten to zero-voltage/zero-current DISABLE. | Preserve a final authority check at the Zephyr driver/hardware-send boundary; queue acceptance is not sufficient for an actuator-enable command. |
| ISR/task visibility of CAN safety and charger completion scalars | **Hardened.** SCE-written CAN/BMS safety scalars and CAN-completion-owned charger/transport diagnostic scalars have explicit volatile visibility. Compound scheduler/mailbox/request identity remains critical-section/request-ID owned. | Preserve explicit ownership. Use atomics/locks or kernel-safe primitives for compound state; do not substitute blanket volatile for synchronization. |
| Recovery commit versus newer physical BOFF | **Fixed in source.** Recovery settlement is tied to the captured BOFF sequence and refuses commit if a newer event appears during HAL/controller checks. | A recovery transaction must name/retire one event epoch and fail if a newer fault event supersedes it. |
| Service recovery followed by a new BOFF before administrative cleanup | **Fixed in source.** `canbus_recover()` atomically commits both transport and application CAN-safety state after proving the BOFF sequence is unchanged. CLI performs no post-return fault/latch cleanup, so a new BOFF after the commit is reasserted by SCE and cannot be erased administratively. | Keep service/reset recovery transactional across driver state and application authority; never split safety-state clearing into a later shell/UI step. |

The focused CAN regression source is expanded to 19 cases, including the new post-success-service-recovery BOFF-survival scenario. **Per user request these final v2.6.27 changes were packaged without rerunning the suites.** The recipient must rerun the focused CAN, safety, comprehensive host, profile/review/contract, and whole-source analysis gates before marking the 0.5.30 candidate frozen.

### 5.3 Closed baseline defects that become migration parity requirements

| Closed item | Baseline status | Migration requirement |
|---|---|---|
| Former F-01 runtime/source version mismatch | **Closed before v2.6.19.** `app.h` aliases `VER_*` to the canonical `ams_version.h`; the behavior was introduced by firmware 0.5.23 and is preserved in the 0.5.30 comparison candidate. | Preserve a single canonical version/build-manifest path and verify shell/CAN/retained/artifact identity in Zephyr. |
...
The vehicle profile enables IWDG, although physical reset timing and persistence evidence remain target-validation gates. Firmware 0.5.30 preserves the closed F-05 policy with an explicit **software-liveness/integrity** policy.
...
- preserve the 0.5.30 uncertainty contract: a calibrated sample has known, nonzero uncertainty; zero and `UINT16_MAX` remain non-calibrated/unknown for estimator, SoP, SoH, and CAN quality decisions;
...
Firmware 0.5.30 freezes the corrected event-time, repeated-event-sequencing, transport-settlement, and atomic service-recovery implementation of the bus-off authority policy that Zephyr must reproduce:
...
The CLI is valuable during bring-up but too large to port wholesale in the first hardware milestone. It contains substantial product-specific diagnostics and uses bounded blocking UART writes from a low-priority thread. The 0.5.30 vehicle profile sets `AMS_ENABLE_CLI=0`, so the vehicle startup/ISR path does not initialize UART3, arm RX, or create the CLI task. Preserve that no-shell vehicle default; target link-map evidence should still confirm unreachable CLI sections are eliminated from the qualified image.
...
1. Freeze firmware 0.5.30 / package v2.6.27 as the FreeRTOS comparison baseline after the delegated final host gates pass and archive the focused measurement-integrity, post-closeout, and migration-readiness regression results. Do not reopen F-01/F-02/F-03/F-04/F-10/F-13 or the v2.6.22 AUX2/freshness/profile-test defects as active blockers unless a later change regresses their gates.
...
4. Produce clean FreeRTOS DEBUG and deterministic RELEASE baseline ELF/MAP/HEX/BIN artifacts on the approved ARM toolchain using 0.5.30 / v2.6.27 with the AMS profile explicitly recorded. Archive `BUILD_PROVENANCE.txt`, `SHA256SUMS.txt`, `.su` files, and the exact toolchain version.
...
7. Archive the approved v2.6.25 CAN bus-off event-time/transport-settlement policy and include its exact 499/500/501 ms and immediate-fail-low vectors in the parity corpus.
...
11. Freeze a list of byte-exact replay vectors and expected safety state transitions, including the v2.6.19/v2.6.20 measurement-integrity edge cases, v2.6.22 constituent-age/AUX2 recovery cases, and v2.6.23 watchdog-policy cases plus the v2.6.24 CAN event-time/service-recovery/tuning-feature cases and v2.6.25 delayed-settlement/fault-injection/soft-error cases.
12. Promote the focused current-window/CAN-aging/uncertainty regressions, v2.6.21 CLI gates, v2.6.22 profile/provenance/AUX2/effective-age gates, and v2.6.23 watchdog-policy, v2.6.24 CAN event-time/service-recovery/tuning gates, and v2.6.25 settlement/injection/error-age gates into the dual-build parity suite where applicable.
...
- [x] Canonical runtime source versioning is unified in the 0.5.30 candidate; the eventual archived target artifact must still be checked against its manifest.
...
1. Rerun the delegated v2.6.27 host gates, then freeze firmware 0.5.30 / package v2.6.27 as the comparison baseline. Archive the v2.6.21-v2.6.26 closeout records plus the v2.6.27 software-freeze-candidate note and the fresh host-validation output with that baseline.
2. Promote the v2.6.19/v2.6.20 current-window, calibration-provenance, CAN per-reading aging, and uncertainty-sentinel cases; the v2.6.22 effective-age/AUX2 cases; and the v2.6.23 watchdog-policy cases and the v2.6.24 CAN ISR/event-time/499-500-501/service-refresh cases and v2.6.25 delayed-settlement/fault-injection/error-age cases into the migration replay/parity corpus.
...
|---|---|
| Canonical firmware version is 0.5.30 in the updated candidate | `Core/Inc/ams_version.h:5-14` |
...
| `measurement/ams_measurement.c` | Preserve | Reuse double-buffer/pinning protocol and 0.5.30 current-window ordering/metadata semantics with injected short lock operations. |
...
Approve the staged Zephyr migration once the delegated v2.6.27 host rerun passes. The STM32F767 and required peripherals are supported, the pure battery algorithms are portable, and the present safety/profile architecture gives a good starting point. The runtime/source version mismatch, code-fixable migration-readiness findings F-02/F-03/F-04/F-10/F-13, the v2.6.22 Release-profile/AUX2/SoP-SoH-freshness/profile-test regressions, safety-policy findings F-05/F-06, v2.6.24 event-time/service/tuning regressions, v2.6.25 transport-settlement/fault-injection/soft-error edge cases, v2.6.26 repeated-BOFF event loss, and the v2.6.27 ISR/task/service-recovery races are closed in the 0.5.30 source candidate. After the recipient confirms the software gates, freeze this FreeRTOS tree and begin the custom-board, no-authority Zephyr Phase 1 skeleton. Hardware/timing/evidence, memory/linker qualification, and target-artifact provenance remain separate authority gates.
