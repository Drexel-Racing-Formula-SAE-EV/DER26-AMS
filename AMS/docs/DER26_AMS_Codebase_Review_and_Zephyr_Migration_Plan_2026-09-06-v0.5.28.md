...
**Reviewed software baseline:** DER26 AMS firmware `0.5.28` / package `v2.6.25`, incorporating the v2.6.17 CAN scheduler corrections, v2.6.19 current-window/measurement-integrity fixes, v2.6.20 follow-up fixes, v2.6.21 migration-readiness closeout, v2.6.22 post-closeout regression fixes, the v2.6.23 watchdog/CAN safety-policy closeout, the v2.6.24 event-time/telemetry/service-recovery fixes, and the v2.6.25 transport-settlement/fault-injection/soft-error/service-recovery hardening
...
**Explicit non-action:** this document update does not start the Zephyr port. The v2.6.25 FreeRTOS baseline preserves the v2.6.23/v2.6.24 watchdog/CAN policy and closes the final clean-pass edge cases: protected completion cannot satisfy the discharge deadline until transport recovery has settled the old controller epoch, bench bus-off injection follows the physical-event path, repeated identical soft CAN errors refresh fault age, and failed manual recovery preserves the repeated-bus-off latch/window. It does not retune battery-model parameters or estimator equations, change CAN IDs/encoding, or change the approved SPI String-B PE4 mapping. No full MiL rerun, ARM target build, flash, physical CAN bus-off test, watchdog-reset timing test, or new hardware validation is claimed for v2.6.25.
...
1. Freeze and fully identify firmware 0.5.28 / package v2.6.25 as the current FreeRTOS comparison baseline.
2. Close the remaining hardware-validation, evidence-reconciliation, target-timing, and target-build-provenance gates; do not reopen issues already fixed and regression-covered in 0.5.28.
...
| Migration authorization | Code-cleared for Phase 1 | Known code-fixable blockers and safety-policy ambiguities are closed in 0.5.28. A no-authority Zephyr skeleton may start; hardware/timing/evidence gates still block parity and vehicle authority. |
...
|---|---|---|
| Former F-01 runtime/source version mismatch | **Closed before v2.6.19.** `app.h` aliases `VER_*` to the canonical `ams_version.h`; the behavior was introduced by firmware 0.5.23 and is preserved in the 0.5.28 comparison baseline. | Preserve a single canonical version/build-manifest path and verify shell/CAN/retained/artifact identity in Zephyr. |
...
The vehicle profile enables IWDG, although physical reset timing and persistence evidence remain target-validation gates. Firmware 0.5.28 preserves the closed F-05 policy with an explicit **software-liveness/integrity** policy.
...
- preserve the 0.5.28 uncertainty contract: a calibrated sample has known, nonzero uncertainty; zero and `UINT16_MAX` remain non-calibrated/unknown for estimator, SoP, SoH, and CAN quality decisions;
...
Firmware 0.5.28 freezes the corrected event-time and transport-settlement implementation of the bus-off authority policy that Zephyr must reproduce:
...
The CLI is valuable during bring-up but too large to port wholesale in the first hardware milestone. It contains substantial product-specific diagnostics and uses bounded blocking UART writes from a low-priority thread. The 0.5.28 vehicle profile sets `AMS_ENABLE_CLI=0`, so the vehicle startup/ISR path does not initialize UART3, arm RX, or create the CLI task. Preserve that no-shell vehicle default; target link-map evidence should still confirm unreachable CLI sections are eliminated from the qualified image.
...
1. Freeze firmware 0.5.28 / package v2.6.25 as the FreeRTOS comparison baseline and archive the focused measurement-integrity, post-closeout, and migration-readiness regression results. Do not reopen F-01/F-02/F-03/F-04/F-10/F-13 or the v2.6.22 AUX2/freshness/profile-test defects as active blockers unless a later change regresses their gates.
...
4. Produce clean FreeRTOS DEBUG and deterministic RELEASE baseline ELF/MAP/HEX/BIN artifacts on the approved ARM toolchain using 0.5.28 / v2.6.25 with the AMS profile explicitly recorded. Archive `BUILD_PROVENANCE.txt`, `SHA256SUMS.txt`, `.su` files, and the exact toolchain version.
...
- [x] Canonical runtime source versioning is unified in the 0.5.28 baseline; the eventual archived target artifact must still be checked against its manifest.
...
1. Freeze firmware 0.5.28 / package v2.6.25 as the comparison baseline. Archive the v2.6.21/v2.6.22 regression records, the v2.6.23 watchdog/CAN policy closeout, and the v2.6.24 final pre-Zephyr code-fix record plus the v2.6.25 freeze-hardening record and host validation with that baseline.
...
|---|---|
| Canonical firmware version is 0.5.28 in the updated baseline | `Core/Inc/ams_version.h:5-14` |
...
| `measurement/ams_measurement.c` | Preserve | Reuse double-buffer/pinning protocol and 0.5.28 current-window ordering/metadata semantics with injected short lock operations. |
