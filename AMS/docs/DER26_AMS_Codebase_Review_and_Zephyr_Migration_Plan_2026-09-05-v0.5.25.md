...
**Reviewed software baseline:** DER26 AMS firmware `0.5.25` / package `v2.6.22`, incorporating the v2.6.17 CAN scheduler corrections, v2.6.19 current-window/measurement-integrity fixes, v2.6.20 follow-up fixes, v2.6.21 migration-readiness closeout, and v2.6.22 post-closeout regression fixes  
...
**Explicit non-action:** this document update does not start the Zephyr port. The v2.6.22 FreeRTOS baseline closes four post-closeout defects: qualified Release/profile parity and provenance, ADBMS AUX2 missed-deadline replay, SoP/SoH constituent-age freshness, and false-positive-prone negative profile gates. It does not retune battery-model parameters or estimator equations, change CAN scheduling policy, watchdog policy, bus-off policy, or the approved SPI String-B PE4 mapping. No full MiL rerun, ARM target build, flash, or new hardware validation is claimed for v2.6.22.
...
1. Freeze and fully identify firmware 0.5.25 / package v2.6.22 as the current FreeRTOS comparison baseline.
2. Close the remaining safety-policy, hardware-validation, evidence-reconciliation, target-timing, and target-build-provenance gates; do not reopen issues already fixed and regression-covered in 0.5.25.
...
- This plan update incorporates the v2.6.21 migration-readiness closeout and the v2.6.22 post-closeout fixes described in Section 5.2a. v2.6.22 intentionally changes stale-data acceptance for SoP/SoH but does not retune battery models or estimator equations.
- No full MiL campaign was rerun. v2.6.22 changes freshness gating, not the estimator/SoP/SoH numerical models or calibration parameters; focused production-adapter/core regressions cover the changed behavior.
- No ARM target binary was built or flashed for v2.6.22 because `arm-none-eabi-gcc` is not available in the review environment.
- No new hardware reproduction is claimed for v2.6.22.
- The focused `measurement-integrity-test` passed the existing groups plus v2.6.22 effective cell/temperature-age, thermal-cadence, unknown-age, and tick-wrap regressions.
...
|---|---|---|
| Former F-01 runtime/source version mismatch | **Closed before v2.6.19.** `app.h` aliases `VER_*` to the canonical `ams_version.h`; the behavior was introduced by firmware 0.5.23 and is preserved in the 0.5.25 comparison baseline. | Preserve a single canonical version/build-manifest path and verify shell/CAN/retained/artifact identity in Zephyr. |
...
- preserve the 0.5.25 uncertainty contract: a calibrated sample has known, nonzero uncertainty; zero and `UINT16_MAX` remain non-calibrated/unknown for estimator, SoP, SoH, and CAN quality decisions;
...
The CLI is valuable during bring-up but too large to port wholesale in the first hardware milestone. It contains substantial product-specific diagnostics and uses bounded blocking UART writes from a low-priority thread. The 0.5.25 vehicle profile sets `AMS_ENABLE_CLI=0`, so the vehicle startup/ISR path does not initialize UART3, arm RX, or create the CLI task. Preserve that no-shell vehicle default; target link-map evidence should still confirm unreachable CLI sections are eliminated from the qualified image.
...
The host test suite and MiL environment are strong assets. The v2.6.22 headless build now has explicit profile selection, CubeIDE-parity Release optimization, controlled Release epoch metadata, dependency/configuration hashes, and an immutable authored-source-input-tree hash. The remaining weakness is **target evidence**, not the source script: no current approved ARM ELF/MAP/HEX/BIN was produced in this review environment.

Before the migration branch:

- record the approved GCC/STM32Cube/HAL/FreeRTOS revisions and preserve the v2.6.22 source-input/provenance manifest;
...
1. Freeze firmware 0.5.25 / package v2.6.22 as the FreeRTOS comparison baseline and archive the focused measurement-integrity, post-closeout, and migration-readiness regression results. Do not reopen F-01/F-02/F-03/F-04/F-10/F-13 or the v2.6.22 AUX2/freshness/profile-test defects as active blockers unless a later change regresses their gates.
...
4. Produce clean FreeRTOS DEBUG and deterministic RELEASE baseline ELF/MAP/HEX/BIN artifacts on the approved ARM toolchain using 0.5.25 / v2.6.22 with the AMS profile explicitly recorded. Archive `BUILD_PROVENANCE.txt`, `SHA256SUMS.txt`, `.su` files, and the exact toolchain version.
...
11. Freeze a list of byte-exact replay vectors and expected safety state transitions, including the v2.6.19/v2.6.20 measurement-integrity edge cases and v2.6.22 constituent-age/AUX2 recovery cases.
12. Promote the focused current-window/CAN-aging/uncertainty regressions, v2.6.21 CLI gates, and v2.6.22 profile/provenance/AUX2/effective-age gates into the dual-build parity suite where applicable.
...
- [x] Canonical runtime source versioning is unified in the 0.5.25 baseline; the eventual archived target artifact must still be checked against its manifest.
...
1. Freeze firmware 0.5.25 / package v2.6.22 as the comparison baseline. Archive the focused measurement-integrity result, the v2.6.21 migration-readiness gates, the v2.6.22 post-closeout regressions, and the v2.6.20-v2.6.22 review records with that baseline.
2. Promote the v2.6.19/v2.6.20 current-window, calibration-provenance, CAN per-reading aging, and uncertainty-sentinel cases plus the v2.6.22 effective-age and AUX2 recovery cases into the migration replay/parity corpus.
...
7. Approve all 27 migration safety invariants and the replay corpus.
...
|---|---|
| Canonical firmware version is 0.5.25 in the updated baseline | `Core/Inc/ams_version.h:5-14` |
...
| `measurement/ams_measurement.c` | Preserve | Reuse double-buffer/pinning protocol and 0.5.25 current-window ordering/metadata semantics with injected short lock operations. |
...
Approve a staged Zephyr investigation, not a product migration commitment. The STM32F767 and required peripherals are supported, the pure battery algorithms are portable, and the present safety/profile architecture gives a good starting point. The runtime/source version mismatch, code-fixable migration-readiness findings F-02/F-03/F-04/F-10/F-13, and the v2.6.22 Release-profile/AUX2/SoP-SoH-freshness/profile-test regressions are closed in the 0.5.25 baseline. The program should now close the remaining safety-policy, hardware, timing/evidence, and target-artifact gates and record the current target behavior. Then build a custom-board, no-authority Zephyr skeleton and port the hardware boundaries under differential and physical test.
