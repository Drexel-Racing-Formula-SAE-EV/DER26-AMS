...
**Reviewed software baseline:** DER26 AMS firmware `0.5.24` / package `v2.6.21`, incorporating the v2.6.17 CAN scheduler corrections, v2.6.19 current-window/measurement-integrity fixes, v2.6.20 follow-up fixes, and the v2.6.21 migration-readiness code closeout  
...
**Explicit non-action:** this document update does not start the Zephyr port. The v2.6.21 FreeRTOS baseline adds only migration-readiness changes: vehicle CLI/UART compile-out, deterministic Release-build metadata enforcement, exact bundled-FreeRTOS provenance pinning, and associated source/host gates. Battery algorithms, CAN scheduling policy, watchdog policy, bus-off policy, measurement authority, and hardware pin mapping are unchanged. No full MiL rerun, ARM target build, flash, or new hardware validation is claimed for v2.6.21.
...
1. Freeze and fully identify firmware 0.5.24 / package v2.6.21 as the current FreeRTOS comparison baseline.
2. Close the remaining safety-policy, hardware-validation, evidence-reconciliation, target-timing, and target-build-provenance gates; do not reopen issues already fixed and regression-covered in 0.5.24.
...
| Migration authorization | Not recommended yet | Start only a no-authority prototype until Phase 0 gates are closed. |
...
- This plan update incorporates the v2.6.21 migration-readiness source/tooling closeout described in Section 5.2; it does not change battery algorithms or safety-policy behavior.
- No full MiL campaign was rerun because the v2.6.21 changes do not touch the algorithm/measurement paths covered by those historical campaigns.
- No ARM target binary was built or flashed for v2.6.21 because `arm-none-eabi-gcc` is not available in the review environment.
- No new hardware reproduction is claimed for v2.6.21.
- The focused `measurement-integrity-test` executable passed all nine named groups after the follow-up fixes.
...
The updated authored AMS `Core` remains approximately **59.2k lines** across **49 C files and 53 headers**. The v2.6.19/v2.6.20 measurement fixes and v2.6.21 migration-readiness guards change only a small number of production lines, so the planning-scale size is unchanged. The complete project remains approximately **114.6k lines** of application code, tests, documentation tools, and support code excluding bundled vendor/middleware/build output.
...
9. The headless **Release** build now refuses to run without `SOURCE_DATE_EPOCH`, records toolchain/source/kernel provenance, and emits artifact hashes. Uncontrolled IDE/Debug builds may still use compiler wall-clock strings and are not the release-reproducibility path.
...
|---|---|---|---|---|
| F-05 | High | Policy decision required | IWDG feeding stops for external battery/sensor faults as well as software liveness faults. A persistent external fault can therefore cause a reset loop after grace time. | Specify whether watchdog responsibility is software integrity only or includes external process faults; test reset-loop and evidence-retention behavior. |
| F-06 | High | Policy decision required | CAN bus-off immediately forces BMS low in charge/HIL, but discharge records a CAN fault without making it a local hard-fault input. This may rely on ECU message staleness. | Write the system safety requirement for bus-off in every state and fault-inject it with ECU behavior present. |
...
| F-13 wall-clock Release metadata | **Closed for the qualified headless Release path.** Release builds now require `SOURCE_DATE_EPOCH`; the build records toolchain/source/FreeRTOS/linker/CubeMX provenance and emits artifact SHA-256 hashes. | Use the deterministic Release path for the FreeRTOS baseline and apply the same controlled-build principle to Zephyr. |
...
|---|---|---|
| Former F-01 runtime/source version mismatch | **Closed before v2.6.19.** `app.h` aliases `VER_*` to the canonical `ams_version.h`; the behavior was introduced by firmware 0.5.23 and is preserved in the 0.5.24 comparison baseline. | Preserve a single canonical version/build-manifest path and verify shell/CAN/retained/artifact identity in Zephyr. |
...
The vehicle profile enables IWDG, but physical target validation remains deliberately gated. The present software feeds during startup grace and during fully healthy operation. It stops feeding not only for missed software heartbeats but for hard battery, charger, ADBMS, fuse, voltage, current, and temperature conditions (`ams_safety.c:858-1015`).

That behavior may be intended, but it must be specified. Resetting on an external persistent fault can:

- repeatedly reboot diagnostics;
- interrupt controlled charger-shutdown publication;
- create a reset loop without improving the external condition;
- complicate root-cause evidence;
- repeatedly exercise hardware startup paths.

Before implementing Zephyr watchdog code, approve one of these policies:

| Policy | Feed stops for | Benefit | Cost |
|---|---|---|---|
| Software-liveness watchdog | missed critical thread, scheduler failure, internal invariant/panic | Resets only when reboot can plausibly restore service. | External faults rely solely on fail-low and diagnostic state. |
| Current broad health watchdog | software failures plus persistent external/process faults | Preserves current behavior exactly. | Can reset-loop on a valid external fault. |
| Two-stage policy | fail low immediately; retain/communicate external fault for a bounded window; stop feed only if internal service is unhealthy or recovery policy expires | Better evidence and deliberate recovery. | More requirements and state-machine work. |

Do not choose during code porting. Freeze the policy first, then write fault-injection tests for every block reason and reset sequence.
...
- preserve the 0.5.24 uncertainty contract: a calibrated sample has known, nonzero uncertainty; zero and `UINT16_MAX` remain non-calibrated/unknown for estimator, SoP, SoH, and CAN quality decisions;
...
The CLI is valuable during bring-up but too large to port wholesale in the first hardware milestone. It contains substantial product-specific diagnostics and uses bounded blocking UART writes from a low-priority thread. The 0.5.24 vehicle profile now sets `AMS_ENABLE_CLI=0`, so the vehicle startup/ISR path does not initialize UART3, arm RX, or create the CLI task. Preserve that no-shell vehicle default; target link-map evidence should still confirm unreachable CLI sections are eliminated from the qualified image.
...
The host test suite and MiL environment are strong assets. Current target build automation is less mature: the headless GCC script is principally an `-O0 -g3 DEBUG` build, no current target ELF/map/hex/bin was found in the reviewed package, and build timestamps are nondeterministic.

Before the migration branch:

- record exact GCC/STM32Cube/HAL/FreeRTOS revisions;
- produce clean DEBUG and RELEASE baseline builds with map files;
...
1. Freeze firmware 0.5.24 / package v2.6.21 as the FreeRTOS comparison baseline and archive the focused measurement-integrity plus migration-readiness regression results. Do not reopen F-01/F-02/F-03/F-04/F-10/F-13 as active blockers unless a later change regresses their gates.
...
4. Produce clean FreeRTOS DEBUG and deterministic RELEASE baseline ELF/MAP/HEX/BIN artifacts on the approved ARM toolchain using 0.5.24. Archive `BUILD_PROVENANCE.txt`, `SHA256SUMS.txt`, `.su` files, and the exact toolchain version.
...
7. Decide and document CAN bus-off behavior in every AMS state.
8. Decide and document watchdog feed/reset-loop policy.
...
11. Freeze a list of byte-exact replay vectors and expected safety state transitions, including the v2.6.19/v2.6.20 measurement-integrity edge cases.
12. Promote the focused current-window/CAN-aging/uncertainty regressions and the v2.6.21 CLI/provenance/profile gates into the dual-build parity suite where applicable.
...
- all 25 migration safety invariants have implementation traces and bench evidence applicable to this phase;
...
- [x] Canonical runtime source versioning is unified in the 0.5.24 baseline; the eventual archived target artifact must still be checked against its manifest.
...
- [x] Qualified headless Release path requires deterministic `SOURCE_DATE_EPOCH` metadata and records provenance/artifact hashes.
...
- [ ] CAN bus-off policy is approved for charge and discharge.
- [ ] Watchdog feed policy is approved.
- [ ] All 25 migration safety invariants and replay corpus are frozen, including current-window boundary, metadata carry, CAN per-reading aging, and calibration-provenance cases.
...
1. Freeze firmware 0.5.24 / package v2.6.21 as the comparison baseline. Archive the focused measurement-integrity result, the v2.6.21 migration-readiness gates, and the v2.6.20/v2.6.21 review records with that baseline.
2. Promote the v2.6.19/v2.6.20 current-window, calibration-provenance, CAN per-reading aging, and uncertainty-sentinel cases into the migration replay/parity corpus.
...
5. Decide the watchdog and CAN bus-off policies. Vehicle CLI policy is already closed: no CLI/UART parser in the vehicle profile.
6. Repair the H-current hardware path and keep other genuine physical gates in a separate visible tracker.
7. Approve all 25 migration safety invariants and the replay corpus.
8. Only then authorize Phase 1: a Zephyr 4.4.0 custom-board, no-authority safe skeleton.

## 18. Source observations supporting key findings

| Observation | Location |
|---|---|
| Canonical firmware version is 0.5.24 in the updated baseline | `Core/Inc/ams_version.h:5-14` |
...
| Release build requires `SOURCE_DATE_EPOCH` and emits provenance plus artifact hashes | `ci/stm32/build_ams_headless_gcc.sh` |
...
| Watchdog blocks on external/process faults | `Core/Src/ext_drivers/ams_safety.c:858-1015` |
...
| `measurement/ams_measurement.c` | Preserve | Reuse double-buffer/pinning protocol and 0.5.24 current-window ordering/metadata semantics with injected short lock operations. |
...
Approve a staged Zephyr investigation, not a product migration commitment. The STM32F767 and required peripherals are supported, the pure battery algorithms are portable, and the present safety/profile architecture gives a good starting point. The runtime/source version mismatch and the code-fixable migration-readiness findings F-02/F-03/F-04/F-10/F-13 are closed in the 0.5.24 baseline. The program should now close the remaining safety-policy, hardware, timing/evidence, and target-build-provenance gates and record the current target behavior. Then build a custom-board, no-authority Zephyr skeleton and port the hardware boundaries under differential and physical test.
