...
The fuse data sheet's typical melting I-squared-t and broad time-current bands
are not used to raise any horizon. v0.3 maintains a subtractive observer using
25% of the 8020 A2s typical melting value, a 300 s decay constant, temperature
derating, and a hottest-surface-plus-15-degC proxy. It remains shadow-only
until `AMS_FUSE_MODEL_VALIDATED=1` and its initialization state is valid. Before
changing that calibration or relying on its derating:

1. Obtain the exact installed fuse lot and manufacturer time-current data.
...
3. Test the complete installed path with calibrated current and temperature
   instrumentation.
4. Keep the independent AMS overcurrent trips below destructive component
